...
if($_POST[Submit] == "修改"){
	...
	if(!empty($file1)){
	@unlink("../../{$rows[image]}");	
$query = "UPDATE `jiaodian` SET `image` = '$file1',`url` = '$_POST[url]',`px` = '$_POST[px]',`times` = '".time()."' WHERE `id` =$id";
	}else{
	$query = "UPDATE `jiaodian` SET `url` = '$_POST[url]',`px` = '$_POST[px]',`times` = '".time()."' WHERE `id` =$id";
	}
	if($db->query($query)){
                ...
                <tr>
                  <td width="46%" valign="middle"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td width="95%"><span class="STYLE3">你当前的位置</span>：[焦点图管理]-[编辑]</td>
                      </tr>
                  </table></td>
                  <td width="54%">&nbsp;</td>
                </tr>
