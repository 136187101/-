...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
		...
		$sUnsafeFileName = time()+mt_rand(1000-2000). strrchr($uploadedFile['name'],'.');
        ...
        $now=time();
        $yearmonthDir=$sServerDir.'/'.date('Ym',$now).'/';
        if (!file_exists($yearmonthDir) && !is_dir($yearmonthDir)){
            mkdir($yearmonthDir,0777);
        }
        $sServerDir=$yearmonthDir;
        $oRegistry->set("FileUpload_url",$this->_currentFolder->getUrl().date('Ym',$now).'/');
