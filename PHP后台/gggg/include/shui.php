<?php
	...
	if($act=="op"){
		...
		$te=imagecolorallocatealpha($im,232,32,192,0);//颜色
		$str=iconv("UTF-8","UTF-8","$title");//确定要绘制的文字，，将gbk转换成UTF-8的编码
		imagettftext($im,22,0,$width/2,$height,$te,"simkai.ttf",$str);//绘制中文，，，绘制的图片，字大小，字角度，x轴，y轴，颜色，字体，内容
		header("Content-type: image/jpeg");//定义要输出的类型
		imagejpeg($im);//加个100可以更清晰
	}
?>
<!--<img src="inc/shui.php?act=op&img=图片的路径&title=//urlencode("好想和你在一起")"/>
-->