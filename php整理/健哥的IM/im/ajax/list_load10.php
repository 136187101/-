...
$sql_num = $db->query("select id from im_list where uid='$euid' and euid='$cookie_xsuid' UNION ALL select id from im_list where uid='$cookie_xsuid' and euid='$euid'");
$num_num = $db->num_rows($sql_num);
$limik = $num_num - 5;
if($limik < 0){$limik = 0;}

if($num_num != 0){

$sql = $db->query("(select id,uid,image,content,duan,time_at from im_list where uid='$euid' and euid='$cookie_xsuid') UNION ALL (select id,uid,image,content,duan,time_at from im_list where uid='$cookie_xsuid' and euid='$euid') ORDER BY id ASC LIMIT $limik,$num_num");
