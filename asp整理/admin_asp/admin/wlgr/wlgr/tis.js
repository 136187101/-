...
//弹出窗口
function jump(url,iname,iWidth,iHeight)
{
    var iTop=(window.screen.height-iHeight)/2;
    var iLeft=(window.screen.width-iWidth)/2;
    window.open(url,iname,"Scrollbars=no,Toolbar=no,Location=no,Direction=no,Resizeable=no,Width="+iWidth+" ,Height="+iHeight+",top="+iTop+",left="+iLeft); 	
}
...
//导航链接跳转
function gourl(rurl,num)
{
	top.document.frames.main.location = rurl;
	if(num==1) top.document.all.middle.cols = '200,150,*';
	else top.document.all.middle.cols = '200,0,*';
}
